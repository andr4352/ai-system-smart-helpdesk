...
1. Чем AI-система отличается от обычной веб-системы? Нужны обучение, версии данных и модели, контроль качества прогнозов.

2. Что такое Train-Serving Skew? Различие обработки при обучении и работе. Предотвращается общим Pipeline.

3. Когда нужен монолит, а когда микросервисы? Монолит — для небольшой команды; микросервисы — для независимого масштабирования и тяжелых моделей.

4. Зачем Pydantic? Проверяет поля, типы и ограничения входа; описывает схему API.

5. Почему веса не хранят в Git? Бинарные версии увеличивают историю. В Git остаются код и указатели на артефакты.

6. Что такое ADR? Запись контекста, решения, обоснования и последствий с датой и статусом.

7. Чем отличаются виды дрейфа? Data Drift — изменение входа; Concept Drift — связи входа с правильным ответом. Проверяются распределениями и новыми метками.

8. Почему /health не всегда возвращает 200? Работающий процесс может не иметь модели или доступа к БД; тогда нужен 503.

9. Как защитить API от неверного ввода? Проверять ключ, схему, длину, размер, частоту и время запросов.

10. Как проследить запрос? По одному request_id в обработчиках, аудите и ответе.
